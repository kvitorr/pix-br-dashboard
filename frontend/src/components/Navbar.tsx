...
export function Sidebar() {
  ...
  return (
    <aside
      className={`flex flex-col h-screen sticky top-0 bg-white border-r border-border shrink-0 transition-all duration-300 ease-in-out ${
        isOpen ? 'w-60' : 'w-sidebar-collapsed'
      }`}
    >
      {/* Header */}
      <div className="flex items-center h-14 px-3 border-b border-border-s overflow-hidden">
        <button
          onClick={toggle}
          className="p-1.5 rounded-md text-secondary hover:text-main hover:bg-subtle transition-colors shrink-0"
        ...
        <span
          className={`ml-3 font-bold text-accent text-sm whitespace-nowrap transition-all duration-300 overflow-hidden ${
            ...
            className={({ isActive }) =>
              `flex items-center rounded-lg px-2 py-2.5 transition-colors overflow-hidden ${
                isActive
                  ...
                  : 'text-secondary hover:text-main hover:bg-subtle'
              }`
            }
          >
            {link.icon}
