...
Calcula os indicadores de adoção per capita e consolida as variáveis socioeconômicas para cada município/mês. É a VIEW principal, alimentando as páginas 1, 2 e 3 do dashboard.
...
Agrega os indicadores mensalmente por região geográfica. Alimenta a página 4 do dashboard (Evolução Temporal).
...
O dashboard é uma aplicação React (Vite) que consome a API REST do Spring Boot e apresenta os dados em quatro páginas temáticas navegáveis. O acesso é público, sem autenticação, via URL permanente.
...
| GET | `/api/dashboard/fatores-socioeconomicos` | `regiao?`, `anoMes?` | `vw_indicadores_municipio` | Dados para scatter plots e correlações |
...
**Objetivo:** Investigar a associação entre a adoção do Pix e variáveis socioeconômicas municipais, apresentando tanto os coeficientes estatísticos quanto a distribuição visual dos dados.

**Componentes visuais:**

| Componente | Tipo | Dado exibido | Campos da VIEW |
|---|---|---|---|
| Badge ρ PIB per capita | Indicador estático | Coeficiente de Spearman pré-calculado (Python) | — |
| Badge ρ IDHM | Indicador estático | Coeficiente de Spearman pré-calculado (Python) | — |
| Badge ρ Urbanização | Indicador estático | Coeficiente de Spearman pré-calculado (Python) | — |
| Scatter: PIB × Penetração | Scatter plot | Um ponto por município, colorido por região | `pib_per_capita` (X), `penetracao_pf` (Y) |
| Scatter: IDHM × Penetração | Scatter plot | Um ponto por município, colorido por região | `idhm` (X), `penetracao_pf` (Y) |
| Scatter: Urbanização × Penetração | Scatter plot | Um ponto por município, colorido por região | `taxa_urbanizacao` (X), `penetracao_pf` (Y) |
| Síntese das Correlações | Bar chart horizontal | Todos os coeficientes ρ por variável, em ordem decrescente | Valores pré-calculados |

> **Nota técnica:** Os coeficientes de Spearman são calculados externamente em Python (`scipy.stats.spearmanr`) e armazenados como valores estáticos no frontend. Os scatter plots fornecem a evidência visual complementar, consumindo dados ao vivo da API.

**Filtros:** Região e mês de referência (afeta os scatter plots; os badges de correlação são valores globais).
